# -*- coding:UTF-8 -*-
'''
Created on 2018年4月12日 16:42:16
@author: 孙留平
'''
...
class TaskManager( unittest.TestCase ):
    ...
    def setUp( self ):
        retClearPolicyList = PolicyListIntf.clear_policy_list()
        self.assertTrue( retClearPolicyList , '清空策略列表失败' )
        pass
    ...
    def test_01AddNormal( self ):
        '''任务管理-新增任务-正常新增'''
        # 添加策略
        cname = "自动化测试策略"
        ename = 'autotestPolicyToDelete'
        toAddNormalPolicy = copy.deepcopy( PolicyPara.addPolicyDict )
        toAddNormalPolicy['cname'] = cname
        toAddNormalPolicy['ename'] = ename
        toAddNormalPolicy['code'] = 'String ss = new String ();'
        resultAddPolicy = PolicyListIntf.add_policy( toAddNormalPolicy )
        self.assertTrue( resultAddPolicy, "=====添加策略失败=====" )
        
        name = 'testTask'
        toAddNormalTask = copy.deepcopy( TaskPara.addDict )
        toAddNormalTask['name'] = name
        toAddNormalTask['taskGroup'] = 'testGroup'
        toAddNormalTask['taskPloyDO.cname'] = cname
        toAddNormalTask['Data'] = '0 0 1 * * * ?'
        
        resultAddTask = TaskListIntf.add_task( toAddNormalTask )
        self.assertTrue( resultAddTask, "添加任务失败" )
        pass
